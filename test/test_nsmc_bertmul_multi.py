...
def main(file_path='review_total.csv', save_path=None):
    ...
    df_org.to_csv(f"./src/data/{sample_type}/{model}/org_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    li_ri = parmap.starmap(ri_proc, input_indices, df, pm_pbar=True, pm_processes=num_cores)
    li_sr = parmap.starmap(sr_proc, input_indices, df, pm_pbar=True, pm_processes=num_cores)

    ### random insertion ###
    df_ri = pd.DataFrame(li_ri)
    ri = df_org.append(df_ri, ignore_index=True)
    ri.to_csv(f"./src/data/{sample_type}/{model}/ri_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    print(f"Checking : length of origin + ri data {len(ri)}")
    ### synonym replacement ###
    df_sr = pd.DataFrame(li_sr)
    sr = df_org.append(df_sr, ignore_index=True)
    sr.to_csv(f"./src/data/{sample_type}/{model}/sr_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    bt.to_csv(f"./src/data/{sample_type}/{model}/bt_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    rs.to_csv(f"./src/data/{sample_type}/{model}/rs_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    rd.to_csv(f"./src/data/{sample_type}/{model}/rd_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    noise.to_csv(f"./src/data/{sample_type}/{model}/noise_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    ...
    total.to_csv(f"./src/data/{sample_type}/{model}/total_{model[:3]}_train_s{sample_type}_{sample_num}.csv", index=False)
    print(f"Checking : length of origin + bt data {len(total)}")


if __name__ == '__main__':
    path = "./src/data/ratings_train_s10000.csv"
    main(file_path=path)