...
def padding_sentence(max_sequence_length, mode):
    ...
    if mode == 'train':
        corpus_file = os.path.join(data_dir, 'train.txt')
    elif mode == 'valid':
        corpus_file = os.path.join(data_dir, 'valid.txt')
    else:
        corpus_file = os.path.join(data_dir, 'ratings_test.txt')
...
def build_dataset():
    ...
    valid.to_csv(data_dir / 'valid.txt', sep='\t', index=False)
