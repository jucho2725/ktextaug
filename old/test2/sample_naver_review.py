import pandas as pd
from old.ktextaug import BackTranslate
...
def check_dup(df: pd.DataFrame, source: pd.DataFrame):
    ...
    print("length ", len(drop_l))
    if not drop_l:
        return df
    else:
        df.drop(inplace=True, axis=0, index=[df.index[i] for i in drop_l])
        df.reset_index(drop=True)
        tmp = source.sample(n=len(drop_l), random_state=random_state)
...
def main(train, test ,num_of_sample):
    ...
    # test2 불러오고 test2 내 pos 와 neg 의 비율 구함
    df_test = pd.read_table(test)
    rate_pos = len(df_test[df_test['label'] == 1]) / len(df_test)

    rs_p = df_pos.sample(n= int(num_of_sample*rate_pos), random_state=random_state)
    rs_p = check_dup(rs_p, df)

    rs_n = df_neg.sample(n= num_of_sample - int(num_of_sample*rate_pos), random_state=random_state)
